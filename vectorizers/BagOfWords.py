...
class BagOfWords:
    ...
    def lower_all(self, data):
        lower_=[]

        for sentence in data:
            lower_.append(sentence.lower())
    ...
    def processText(self,text_data):

        lower_text=self.lower_all(text_data)

        remove_punctuation = []

        for word in lower_text:
            remove_punctuation.append(self.remove_punctuation(word).strip())
    ...
    def fit(self,x_):
        ...
        self.words = self.processText(x_)
...
regressionModel.fit(X,labels)


email=model.transform(["Free Iphone win"])
#print(email)


print(regressionModel.predict(email[0]))
